...
namespace Functions_Logic.Tests.Importers.Tests
{
    ...
    public partial class FxCsvImporterTests
    {
        ...
        [Scenario]
        [TestCaseSource(nameof(GetSpuriousCsvImportData))]
        public void Import_spurious_data_test(string path)
        {
            Runner.RunScenario(
                Given => a_valid_csv_path_with_spurious_data(path),
        ...
        public void Import_valid_file_test(string filePath, object maxHor, object maxSim, int horExpectd, int simExpected)
        {
            Runner.RunScenario(
                ...
                Then => I_can_get_the_fx_array_from_cache(horExpectd, simExpected));
        }
